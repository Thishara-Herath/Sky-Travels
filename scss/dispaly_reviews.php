...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_review'])) {
	...
	$stmt = $conn->prepare("UPDATE reviews SET confirmed = TRUE WHERE id = ?");
	$stmt->bind_param("i", $reviewId);
	if ($stmt->execute()) {
		echo '<script>alert("Review confirmed successfully.");</script>';
	} else {
		echo '<script>alert("Error confirming review: ' . $stmt->error . '");</script>';
	}
	$stmt->close();
}
?>
...
<!-- Nav Item - Tables -->
            <li class="nav-item active">
                <a class="nav-link" href="dispaly_reviews.php">
            ...
            <i class="fas fa-sign-out-alt fa-sm fa-fw mr-2 text-gray-400"></i> Logout
				...
				<div class="container-fluid p-0">
					...
					if ($result->num_rows > 0) {
						echo '<h1>Reviews</h1>';
						echo '<table>';
						echo '<tr><th>Name</th><th>Designation</th><th>Profile Photo</th><th>Message</th><th>Actions</th></tr>';
						...
						while ($row = $result->fetch_assoc()) {
							...
							// Only show the confirm button if the review is not confirmed
							if (!$row['confirmed']) {
								echo '<form action="" method="POST" style="display:inline;">
                        <input type="hidden" name="review_id" value="' . htmlspecialchars($row['id']) . '">
                        <input type="submit" name="confirm_review" value="Confirm">
                      </form>';
							} else {
								echo 'Confirmed';
							}
